...
export default async function Experiences() {
  ...
  return (
    <div className="bento-card p-5 flex flex-col gap-5 group relative overflow-hidden backdrop-blur-xl bg-card/50 border-border flex-1">
      ...
      <div className="flex flex-col gap-4">
        {experiences.slice(0, 3).map((exp) => (
          <div key={exp.id} className="space-y-1.5 border-l border-border pl-4 min-w-0">
            <h4 className="text-[12px] font-bold text-foreground uppercase tracking-wide leading-none truncate">{exp.title}</h4>
