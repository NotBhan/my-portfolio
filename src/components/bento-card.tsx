...
export default function BentoCard({ children, className, title, as: Component = 'div', showButtons = false }: BentoCardProps) {
  return (
    <Component
      className={cn(
        'relative flex w-full flex-col rounded-xl border bg-card/50 backdrop-blur-sm text-card-foreground shadow-sm transition-all duration-300 ease-in-out hover:shadow-lg hover:-translate-y-1',
        className
      )}
    >
