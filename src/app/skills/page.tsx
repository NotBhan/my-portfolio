  ...
  return (
    <div className="space-y-6 pt-4">
      <div className="flex items-center gap-3 mb-12">
        ...
        <div>
          <h1 className="text-2xl font-black text-foreground uppercase tracking-tight">Capabilities</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-12">
        {skillCategories.map((cat) => (
          <div key={cat.category} className="bento-card p-8 flex flex-col gap-6 bg-card/50">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-xl font-black text-foreground uppercase tracking-[0.1em]">{cat.category}</h2>
            </div>

            <div className="flex flex-wrap gap-3">
              {cat.skills.filter(s => s.isVisible).map((skill) => (
                <div key={skill.name} className="flex flex-col gap-1.5 group">
                  <Badge className="px-5 py-2.5 bg-muted/20 border-border text-sm text-foreground hover:bg-primary/20 hover:text-primary transition-all rounded-xl cursor-default">
                    {skill.name}
                  </Badge>
                </div>
              ))}
