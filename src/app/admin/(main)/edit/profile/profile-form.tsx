...
export default function ProfileForm({ profile: initialProfile }: { profile: Profile }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Success', description: 'Profile changes synced.' });
      router.refresh();
    } catch (error) {
      toast({
        title: 'Sync Error',
        description: 'Could not update profile. System reported an error.',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Button 
        type="submit" 
        className="w-full h-12 rounded-xl bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-[0.2em] shadow-lg shadow-primary/20"
        disabled={isSaving}
      >
        {isSaving ? 'Syncing...' : 'Sync Profile'} <Save className="ml-2 h-4 w-4" />
      </Button>
    </form>
  );
}
